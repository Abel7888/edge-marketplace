...
import { Menu, X, Zap, ChevronDown, User, Settings, LogOut, Home } from 'lucide-react'
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
        {/* Left: Logo */}
        <a href="/" className="flex items-center focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-lg flex-shrink-0 hover:opacity-80 transition-opacity">
        ...
        <div className="ml-auto flex items-center gap-2">
          {!currentUser && (
            <>
              <a href="/login" className="hidden md:inline-flex items-center justify-center px-3 h-10 rounded-lg border-2 border-gray-200 text-sm font-semibold text-gray-800 hover:bg-gray-50">Sign In</a>
              <a href="/signup" className="hidden md:inline-flex items-center justify-center px-3 h-10 rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-blue-500 via-blue-600 to-cyan-500 hover:shadow-lg">Sign Up</a>
            </>
          )}
          {currentUser && (
            <>
              <a href="/discover" className="hidden md:inline-flex items-center justify-center gap-2 px-4 h-10 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-sm font-semibold hover:shadow-lg transition-all">
                <Home size={16} />
                Home
              </a>
          ...
          {currentUser && (
            <div className="relative">
              ...
              <AnimatePresence>
                {profileOpen && (
                  <motion.div initial={{opacity:0, y:-6}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-6}} className="absolute right-0 mt-2 bg-white rounded-xl shadow-xl w-64 p-2 z-50">
                    <div className="px-3 py-2">
                      <div className="font-semibold text-slate-900">{currentUser.displayName||'User'}</div>
                      <div className="text-sm text-gray-600">{currentUser.email}</div>
                    </div>
                    <div className="my-2 h-px bg-gray-100" />
                    <button onClick={async () => { try { await logout(); navigate('/'); } catch {} }} className="w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-red-50 text-red-600"><LogOut size={16} /> Sign Out</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.aside
            ...
            className="lg:hidden fixed right-0 top-0 bottom-0 w-80 bg-white shadow-xl p-6 z-50"
            role="dialog"
            aria-modal="true"
          >
            ...
            <div className="mt-8 space-y-3">
              {!currentUser && (
                <>
                  <a href="/login" className="w-full inline-flex items-center justify-center px-4 py-3 rounded-xl border border-gray-200 font-semibold text-gray-800 hover:bg-gray-50">Sign In</a>
                  <a href="/signup" className="w-full inline-flex items-center justify-center px-4 py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-blue-500 via-blue-600 to-cyan-500 hover:shadow-lg">Sign Up</a>
                </>
              )}
              {currentUser && (
                <>
                  <div className="px-2 text-sm text-gray-700">Welcome, {String(currentUser.displayName||currentUser.email||'User').split(' ')[0]}</div>
                  <button onClick={async () => { try { await logout(); navigate('/'); } catch {} }} className="w-full inline-flex items-center justify-center px-4 py-3 rounded-xl border font-semibold text-gray-800 hover:bg-gray-50">Logout</button>
                </>
              )}
            </div>
          </motion.aside>
